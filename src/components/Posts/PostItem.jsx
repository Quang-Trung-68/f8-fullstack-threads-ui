...
import UserProfileView from "../Feeds/UserProfileView";
import PostDetailView from "../Feeds/PostDetailView";

function PostItem({ userId, id, title, body, isPermitDetailPost, navigation }) {
  const navigate = useNavigate();
  const handleToPostDetail = () => {
    if (isPermitDetailPost) {
      if (navigation) {
        navigation.push(<PostDetailView id={id} />);
      } else {
  ...
  const handleToUserProfile = () => {
    if (navigation) {
      // Extract userId from somewhere or use a mock one since it's not passed explicitly in some cases
      // Assuming userId is available as prop
      navigation.push(<UserProfileView userId={userId || 1} />);
    } else {
      navigate(`/@${userId}`);
    }
  };
