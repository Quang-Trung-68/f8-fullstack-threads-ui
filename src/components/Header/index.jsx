...
export default function Header() {
  ...
  const getCurrentTitle = () => {
    // Flatten routes to find match
    const allRoutes = ROUTES.flatMap((r) => r.children);
    const currentRoute = allRoutes.find((r) => r.path === location.pathname);
    return currentRoute?.title || "Threads";
  };

  const isHome = location.pathname === "/";

  return (
    <div className="fixed z-50 top-0 right-0 left-0 flex items-center justify-between bg-white backdrop-blur-md bg-opacity-80 md:bg-white/80">
      ...
      <div className="hidden md:block">
        {isHome ? (
          ...
          <div className="text-xl font-bold">{getCurrentTitle()}</div>
        )}
      </div>
